...
CREATE TABLE IF NOT EXISTS public.profiles (
    ...
    reg TEXT, 
    specialty TEXT,
    plan_type TEXT DEFAULT 'basico' CHECK (plan_type IN ('basico', 'medio', 'avancado')),
...
CREATE TABLE IF NOT EXISTS public.medical_records (
    ...
    history TEXT,
    expires_at TIMESTAMPTZ, -- 48 hours auto expiry for privacy & storage optimization
    created_at TIMESTAMPTZ DEFAULT NOW(),
    updated_at TIMESTAMPTZ DEFAULT NOW()
);
...
CREATE POLICY "Appointments Shared Access" ON public.appointments FOR ALL USING (auth.uid() = patient_id OR auth.uid() = clinic_id);
